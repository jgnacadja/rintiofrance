<template>
  ...
  ]" class="xl:px-12 xl:py-16 2xl:px-32 2xl:py-24">
    <div
      class="grid grid-cols-6 gap-4 px-4 py-8 md:grid-cols-4 lg:grid-cols-3 md:grid-flow-row lg:grid-flow-rows"
      >

      <div class="w-full h-full col-span-6 row-span-4 md:row-span-6 md:col-span-4 lg:col-span-1 lg:row-span-2">
        <div class="">
          <div class="">
          ...
          <div class="pt-6">
            <h2 class="my-3 text-2xl font-extrabold md:text-5xl text-primary" v-html="content.title"></h2>

            <div class="my-3 font-light" v-html="richtextToHTML(content.text)"></div>
            <div class="mt-4 text-center">
              <g-link :to="button.ctaLink">
                <button v-if="button.ctaText" type="submit"
      ...
      <div class="w-full h-full col-span-6 row-span-2 md:col-span-2 md:row-span-2 lg:col-span-1 lg:row-span-2">
        <div class="relative bg-center bg-no-repeat bg-cover shadow-lg group h-80 md:h-full"
          ...
          <div class="absolute px-4 bottom-4">
            ...
            <div class="text-xs font-bold leading-relaxed text-white font-roboto"
              v-html="richtextToHTML(edge.node.metaDescription)"></div>

            <div
              class="text-white tracking-wider leading-relaxed font-roboto text-xs flex flex-row justify-between -mt-5 space-x-4"
              v-html="edge.node.excerpt"></div>
          </div>
        </div>
      </div>
        ...
        <div class="bg-white bg-center bg-no-repeat bg-cover shadow-lg group" v-bind:style="[
          ...
          <div class="w-full">
            ...
            <div class="relative w-full px-4 pb-4">
              ...
              <div class="text-xs font-bold leading-relaxed tracking-wider text-white font-roboto"
                v-html="richtextToHTML(edge.node.metaDescription)"></div>

              <div class="text-xs leading-relaxed tracking-wider text-white"
                v-bind:class="{ '-mt-5': edge.node.metaDescription }" v-html="edge.node.excerpt"></div>
            </div>
          </div>
        </div>
      </div> 
